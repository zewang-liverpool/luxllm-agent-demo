...
| `llm\_fresh`     | A fresh LLM decision was used                       |

| `cached\_llm`    | A recent LLM plan was reused                        |

| `fallback`      | General fallback behaviour was used                 |

| `rule\_fallback` | Rule-based fallback repaired or replaced a decision |

| `rule\_player`   | Rule-based player logic produced the action         |

| `rule\_only`     | Rule-only mode was active                           |
...
| player\_0 wins           | Number of matches won by player\_0        |

| player\_1 wins           | Number of matches won by player\_1        |

| Draws                   | Number of matches without a clear winner |

| player\_0 win rate       | player\_0 wins divided by total runs      |

| Average player\_0 reward | Mean reward for player\_0                 |

| Average player\_1 reward | Mean reward for player\_1                 |
...
The 70% player\_0 win rate suggests that the qwen3:32b-backed configuration produced stronger gameplay outcomes than the comparison DeepSeek-R1-32B configuration in the current evaluation setup.
...
The player\_0 win rate was 52%, which is lower than the qwen3:32b result but still demonstrates stable controlled execution.
...
Trace records may show that behaviour came from `rule\_only`, `fallback`, or `rule\_fallback`. This is useful for stability, but it means final actions cannot always be attributed directly to the LLM.
...
Second, the failure-case analysis is currently representative rather than exhaustive. Future work could automatically identify and classify failure cases from logs.



Third, fallback counts can be difficult to interpret because fallback may be recorded at different levels, such as unit-level fallback or decision-source fallback.



Fourth, the replay overlay provides strong qualitative evidence, but it depends on correct alignment between replay frames and decision traces.



Fifth, the current system evaluates framework stability and inspectability more strongly than optimal gameplay performance.
...
The system records decision-source information such as `llm\_fresh`, `cached\_llm`, `fallback`, `rule\_fallback`, and `rule\_player`.
