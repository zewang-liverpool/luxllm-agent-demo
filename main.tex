...
The current controlled experiments use a local Ollama backend with \texttt{qwen3:32b} served on Barkla2 GPU resources. The LLM is queried sparsely rather than on every frame. Fresh LLM decisions are recorded as structured JSON-like decisions, while cached LLM decisions are reused across later turns when appropriate. This design reduces repeated model calls and makes it possible to compare fresh model decisions, cached decisions, and fallback events in the same replay trace.
...
The main strategy-diverse prompting configuration uses compact structured LLM decisions, strategy-diverse prompting, deterministic rule fallback, and cached LLM decisions between fresh model calls. In a 50-match controlled evaluation, the LLM-assisted player won 29 matches against 21 wins for the rule-controlled opponent. The run completed with zero average LLM errors and complete trace coverage, with an average of 1010 logged trace steps per match.

We also evaluated a candidate-exploitation ablation. This variant modified target selection to encourage more explicit use of candidate scoring tiles. In a separate 50-match evaluation, it achieved 26 wins for the LLM-assisted player and 24 wins for the rule-controlled opponent. Although the ablation remained technically stable, it did not outperform the strategy-diverse prompting configuration at the 50-match scale. We therefore use the strategy-diverse prompting configuration as the main controlled result and report candidate exploitation as an ablation.
...
\hline
Configuration & Runs & LLM-assisted wins & Rule-opponent wins & Avg. LLM errors & Avg. trace steps \
\hline
Strategy-diverse prompting & 50 & 29 & 21 & 0.0 & 1010.0 \
Candidate-exploitation ablation & 50 & 26 & 24 & 0.0 & 1010.0 \
\hline
\end{tabular}
\caption{Controlled \texttt{qwen3:32b} evaluation on Barkla2. The strategy-diverse prompting configuration is used as the main result, while candidate exploitation is reported as an ablation.}
\label{tab:qwen3-controlled-eval}
\end{table*}

For the main configuration, the decision-source logs contain 787 fresh LLM decisions, 24463 cached LLM decisions, and 25250 rule-player actions across the 50-match run. The candidate-exploitation ablation contains 1277 fresh LLM decisions, 23973 cached LLM decisions, and 25250 rule-player actions. These statistics show that the system is not merely calling an LLM once for a static plan, nor is it relying on a fresh model call at every frame. Instead, it combines sparse LLM strategy generation with cached reuse and fallback-safe execution.
...
\begin{tabular}{lrrrr}
\hline
Configuration & Fresh LLM decisions & Cached LLM decisions & Rule-player actions & Total sources \
\hline
Strategy-diverse prompting & 787 & 24463 & 25250 & 50500 \
Candidate-exploitation ablation & 1277 & 23973 & 25250 & 50500 \
\hline
\end{tabular}
\caption{Decision-source statistics across the two 50-match controlled runs. Fresh LLM counts indicate newly generated model decisions, cached LLM counts indicate reused strategies, and rule-player counts correspond to the deterministic opponent.}
\label{tab:decision-source-stats}
\end{table*}

The results should be interpreted as evidence of controlled runtime stability and inspectability rather than as proof of optimal game-playing strength. The strategy-diverse prompting configuration achieved the stronger 50-match result, while the candidate-exploitation ablation remained stable but did not improve the win count. This supports a useful negative finding: adding more explicit candidate-target exploitation does not automatically improve long-horizon performance in this environment. The ablation is therefore valuable because it demonstrates that the system can compare design variants using the same trace and evaluation pipeline.
...
The demo package is intended to be released with source code, replay artefacts, screenshots, evaluation summaries, and a short screencast video. The final public repository and video links will be added before submission. The system uses the Lux AI Season 3 environment and local model inference through Ollama. The code and artefacts will be distributed for research and educational use, subject to the licences of the underlying environment and dependencies.

\section{Conclusion}

LuxLLM-Agent demonstrates an inspectable LLM-assisted agent pipeline for Lux AI Season 3. The system combines local LLM strategy generation, rule-based fallback, structured decision logging, replay-driven visualisation, controlled evaluation, and lightweight scalability evidence. The current \texttt{qwen3:32b} evaluation shows that the system can run stable 50-match controlled experiments with complete trace coverage and zero average LLM errors, while the scalability simulation shows how sparse LLM-generated policy templates can be reused across many lightweight worker agents. The demonstration is intended to help researchers and students inspect, reproduce, and discuss LLM-assisted game-agent behaviour.
...
The local development artefacts include replay JSON, Markdown reports, screenshots, demo video material, and LaTeX integration files. The final replay viewer was verified with 505 replay frames, 96.83% both-player frame coverage, 662 risk-filter changes, and a compiled paper artefact. The scalability simulation was verified up to 1000 lightweight worker agents and 100000 synthetic decisions.
