...
\begin{abstract}
We present LuxLLM-Agent, an inspectable LLM-assisted agent system for Lux AI Season 3. The system combines a local game-playing agent, rule-based fallback policies, structured decision logs, and an isometric battle replay viewer for analysing agent behaviour. Unlike a standard replay viewer, our interface connects game states with decision traces, cached LLM decisions, fallback events, match-level scores, and final result summaries. In separate 50-match controlled runs on Barkla2, a target-aware \texttt{qwen3:32b} planner achieved a descriptive win rate of 70\%, compared with 56\% for a previous basic planner, while both configurations recorded zero LLM errors. The system also includes a lightweight scalability simulation with up to 1000 worker agents. The demo package includes a replay viewer, screenshots, replay JSON, evaluation reports, and screencast material for research and educational use.
\end{abstract}
...
In all reported controlled runs, the LLM-assisted configuration occupied the \texttt{player\_0} role and the rule-controlled opponent occupied \texttt{player\_1}. The roles were not swapped under matched seeds, and the study does not report confidence intervals or hypothesis tests. The win rates are therefore descriptive outcomes for these configurations and should not be interpreted as statistically significant or as isolating a causal effect of the planner from possible player-side and environment effects.
...
LuxLLM-Agent demonstrates an inspectable LLM-assisted agent pipeline for Lux AI Season 3. The system combines local LLM strategy generation, rule-based fallback, structured decision logging, replay-driven visualisation, controlled evaluation, and lightweight scalability evidence. In the reported separate \texttt{qwen3:32b} evaluations, the target-aware planner achieved a descriptive win rate of 70\%, compared with 56\% for the previous basic planner, while both configurations maintained zero LLM errors; the scalability simulation additionally illustrates how sparse LLM-generated policy templates can be reused across many lightweight worker agents. The demonstration is intended to help researchers and students inspect, reproduce, and discuss LLM-assisted game-agent behaviour.
